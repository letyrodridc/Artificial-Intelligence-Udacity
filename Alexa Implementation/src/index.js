...
var handlers = {
    ...
    'GetNewYearFactIntent': function () {
        ...
        var message;  
        var speechOutput;
        var notFoundMessages = ["No AI news in "+year, "I don't know nothing in year "+year, "None fact was found in "+year];
        ...
        // Filter the facts by year
        
        for (var i = 0; i < factArr.length; i++) {
            if (factArr[i].indexOf(year) >= 0) {
                factArrYear.push(factArr[i]);
            }
            
        }

        // Check if a fact was found to that year.
        if (factArrYear.length > 0) {
            // A fact was found in the year, selects random
            message = randomPhrase(factArrYear);
            // Create speech output
            speechOutput = this.t("GET_FACT_MESSAGE") + message;
        } else {
            message = randomPhrase(notFoundMessages);
            // Create speech output
