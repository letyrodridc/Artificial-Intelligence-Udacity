...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Create the action level
        self.a_levels.append(set())

        # Current state
        previous_state = self.s_levels[level]
        clauses_pos = [s.symbol for s in previous_state if s.is_pos]
        clauses_neg = [s.symbol for s in previous_state if not s.is_pos]
        ...
        # Sets children/parent relations
        for s in self.s_levels[level]:
            for a in self.a_levels[level]:
                if s.literal in a.action.precond_pos or s.literal in a.action.precond_neg:
                    s.children.add(a)
                    a.parents.add(s)
    ...
    def add_literal_level(self, level):
        ...
        previous_action = self.a_levels[level-1]
        ...
        for a in self.a_levels[level-1]:
            for s in self.s_levels[level]:
                if s.literal in a.action.effect_add or s.literal in a.action.effect_rem:
                    a.children.add(s)
                    s.parents.add(a)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        eff_a1 = set(node_a1.action.effect_add)
        eff_a2 = set(node_a2.action.effect_rem)

        return len(eff_a1.intersection(eff_a2)) > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        eff_a1_add = set(node_a1.action.effect_add)
        eff_a1_rem = set(node_a1.action.effect_rem)
        eff_a2_add = set(node_a2.action.effect_add)
        eff_a2_rem = set(node_a2.action.effect_rem)

        precond_a1_pos= set(node_a1.action.precond_pos)
        precond_a1_neg = set(node_a1.action.precond_neg)
        precond_a2_pos= set(node_a2.action.precond_pos)
        precond_a2_neg = set(node_a2.action.precond_neg)

        return len(eff_a1_rem.intersection(precond_a2_pos)) > 0 or len(eff_a1_add.intersection(precond_a2_neg)) > 0   or len(eff_a2_rem.intersection(precond_a1_pos)) > 0 or len(eff_a2_add.intersection(precond_a1_neg)) > 0
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return  (node_s1.symbol == node_s2.symbol) \
                   and (not node_s1.is_pos == node_s2.is_pos)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        parents1 = node_s1.parents
        parents2 = node_s2.parents

        all_mutex = True

        for p1 in parents1:
            for p2 in parents2:
                if not (p1.is_mutex(p2) or p2.is_mutex(p1)):
                    all_mutex = False
